...
const { reward_bxg } = require("../models/reward_bxg");
...
router.post("/", async (req, res) => {
  try {
    ...
    const checkInReward = await reward_bxg.findOne({
      where: { wallet_address: req.body.wallet_address },
    });
    console.log("reward data is here ", checkInReward);
    ...
    if (!checkInReward) {
       await reward_bxg.create({
        wallet_address: req.body.wallet_address,
        bxg: parseFloat(0),
      });
    }
    if (!checkInBxgToken) console.log("data here", req.body.wallet_address);
